...
public final class LaLTrimItemModels {
    ...
    private static void registerItemModelModification(String armorPieceType, ResourceLocation armorMaterial) {
        Mixson.registerEvent(
                ...
                LaLConstants.id("add_trims_to_" + armorMaterial.getPath() + "_" + armorPieceType).toString(),
                (context) -> {
                    ...
                    LaLTrimMaterials.TRIM_MATERIALS.forEach(trimMaterial -> {
                        JsonObject newCase = baseCase.deepCopy();

                        newCase.addProperty("when", trimMaterial.location().toString());
                        newCase.getAsJsonObject("model").addProperty("model", LaLConstants.id("item/" + armorMaterial.getPath() + "_" + armorPieceType + "_" + trimMaterial.location().getPath() + "_trim").toString());
    ...
    private static void registerAtlasModification(String atlasName) {
        Mixson.registerEvent(
                ...
                LaLConstants.id("add_trims_to_" + atlasName + "_atlas").toString(),
                (context) -> {
                    JsonObject rootJson = context.getFile().getAsJsonObject();
                    ...
                    for (JsonElement sourceElement : sourcesArray) {
                        ...
                        if ("paletted_permutations".equals(sourceObject.get("type").getAsString())) {
                            ...
                            LaLTrimMaterials.TRIM_MATERIALS.forEach(trimMaterial -> {
                                ...
                                permutationsJson.addProperty(trimName, LaLConstants.id("trims/color_palettes/" + trimName).toString());
                            });
